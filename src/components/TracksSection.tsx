...
const TrackCard = ({ data }: { data: (typeof trackData)[number] }) => (
  <Card
    className={`relative w-full bg-gray-900/70 border-none border-2 border-blue-50 border-${data.color}`}
  >
    <div
      className={cn(
        "absolute inset-0 bg-gradient-to-r rounded-lg",
    ...
    <div className="bg-black py-20">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="my-8">
          ...
          >
            <TabsList className="w-full h-auto bg-transparent justify-between p-0 space-x-4">
              {agentTabs.map((tab) => {
                const Icon = tab.icon;
                return (
                  <TabsTrigger
                    key={tab.value}
                    value={tab.value}
                    className="flex-1 flex items-center justify-center gap-2 px-4 py-3 text-gray-400 data-[state=active]:text-white data-[state=active]:bg-black data-[state=active]:shadow-inner transition-colors duration-200 bg-black"
                  >
